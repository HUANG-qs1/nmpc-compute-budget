...
- `experiments/` — experiment runners and frozen artifacts, one directory per
  ...
  - `exp07_mujoco_graft/` — quadrotor (MuJoCo) main experiment, 1080 runs
    (seeds 230–244): runner `run_graft.py` (all five arms: v3, reactive,
    fixed20, fixed10, best-so-far), batch drivers `drive_batch.py` and
    `drive_exp10.py` (subprocess-isolated, resumable driver for the exp10
    extension arms), graft layer `quad_nmpc.py`, plant `plant.py`, module
    self-checks (`test_plant.py`, `test_nmpc.py`, `test_cap_equiv.py`),
    gear-scan probes (`scan_gears.py`, `probe_iters.py`), watchdog patch
    (`patch_watchdog.py`), bite check (`verify_bite.py`), debug probe
    (`debug_mm.py`), render pipeline (`render_beat.py`, `replay_render.py`,
    `replay_render_v1_backup.py`), verdict instrument `analyze_graft.py`,
    verdict file `verdicts_exp07_srv2.txt`, and the frozen CSV (sharded, see
    below); also hosts `exp10_quad.csv` (720 runs, preregistered extension
    arms fixed10 and best-so-far) and the recorded-trace supplement exp11
    (`record_trace.py`, `replay_trace.py`, `verify_trace_supplement.py`, the
    six recorded budget traces `traceA.csv`–`traceF.csv`, and
    `exp11_trace_replay.csv`; see the exp11 section below).
...
The recorded-trace supplement **exp11** is not part of the frozen 2760-run
dataset. It is a post-hoc supplement: six budget traces recorded from the real
OS scheduler on the experiment host under controlled CPU contention, replayed
through the frozen exp07 closed loop (3 arms × 6 traces = 18 runs). It carries
no preregistered propositions and enters no test family.
...
To regenerate the exp10 extension-arm runs themselves (subprocess-isolated,
resumable; the frozen `exp10_quad.csv` / `exp10_wmr.csv` remain the
experiments of record):

```bash
cd experiments/exp07_mujoco_graft
python3 drive_exp10.py OUTCSV [HOURS]
```

## Recorded-trace supplement (exp11)

exp11 replays six recorded budget traces through the frozen exp07 closed loop
(circle task, seed 230, mismatch 0.0; arms fixed20, best-so-far, v3; 18 runs).
The exp11 files live inside `experiments/exp07_mujoco_graft/` because the
replay driver imports the frozen runner modules unchanged.

- `record_trace.py` — probe-thread recorder: logs the per-beat duty cycle of
  a probe thread under controlled CPU contention.
- `traceA.csv`–`traceF.csv` — six recorded traces, 1050 beats each, recorded
  on 2026-09-18 on the experiment host. The `duty` column is the measured
  quantity. Re-recording produces new, load-dependent traces, so the archived
  files are the experiment of record.
- `replay_trace.py` — replay driver: scales each recorded trace through the
  frozen calibration law and replays it through the frozen closed loop.
- `exp11_trace_replay.csv` — the 18 run-level verdict rows.
- `verify_trace_supplement.py` — eight-check audit over the traces, the
  run-level CSV, and the per-beat log.

Reproduce (about 45 minutes for the replay):

```bash
cd experiments/exp07_mujoco_graft
python3 replay_trace.py traceA.csv traceB.csv traceC.csv traceD.csv traceE.csv traceF.csv
python3 verify_trace_supplement.py   # prints ALL CHECKS PASS
```

The per-beat log `exp11_trace_beats.csv` (18000 rows) is required by the
audit script and is distributed with the Zenodo archive (see below), not in
this repository.
...
logs and the exp11 per-beat log `exp11_trace_beats.csv`) will be linked here
upon archiving.
